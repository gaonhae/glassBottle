...
create index if not exists idx_analytics_events_family_id on public.analytics_events (family_id);
...
values
  (1, '우리 가족과 먹었던 음식 중 가장 기억에 남는 메뉴는 무엇인가요?'),
  (2, '아주 어릴 때, 나는 커서 어떤 사람이 되고 싶었는지 기억나시나요?'),
  (3, '우리가 가봤던 가족 여행지 중에서 딱 한 곳만 다시 갈 수 있다면 어디인가요?'),
  (4, '이번 주말에 우리 가족이 다 같이 딱 한 시간만 시간을 보낸다면 무엇을 하고 싶나요?'),
  (5, '최근에 본 영화나 드라마 중에서 가족들에게 추천하고 싶은 작품이 있나요?'),
  (6, '내가 생각하는 우리 가족만의 특별한 장점이나 매력은 무엇인가요?'),
  (7, '일주일의 휴가가 주어진다면 아무 걱정 없이 꼭 해보고 싶은 일은 무엇인가요?'),
  (8, '길을 걷다 마주쳤을 때 기분이 좋아지는 풍경이나 사물이 있나요?'),
  (9, '요즘 내가 일상에서 가장 중요하게 생각하는 가치는 무엇인가요?'),
  (10, '가족들과 함께 찍은 사진 중 내가 가장 좋아하는 사진은 무엇인가요?')
